...
```{r summary of reg1, echo=FALSE, results='asis'}
# summarytable1 <- data.frame(stargazer(reg1, type = "text"))

summarytable1 <- tidy(reg1)
...
```{r summary of training regression, include = TRUE}
...
kable(
  ...
  caption = "Test Set Error Metrics"
)
```
...
We still need a map of mean absolute percent error by neighborhood

We need the scatterplot of MAPE by neighborhood of mean price by neighborhood
...
There is something so strange going on in this one zip code. Clearly the model is confused about something. The map is at least plotting what is required, but it shows that there is a gap in our approach.
