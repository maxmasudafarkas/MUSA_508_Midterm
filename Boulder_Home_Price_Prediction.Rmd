...
```{r schools, include=TRUE, warning=FALSE, message=FALSE}
boulder_schools <-
...
```{r school density maps, include=TRUE, warning=FALSE, message=FALSE}
...
``` {r schools nearest neighbor correlation, include=TRUE, warning=FALSE, message=FALSE}

st_c <- st_coordinates

boulder_homes_observed <-
  boulder_homes_observed %>% 
    mutate(
      schools_nn3 = nn_function(st_c(boulder_homes_observed), st_c(boulder_schools), 3))
...
feature_list <-
  c("price", "designCodeDscr", "qualityCodeDscr", "nbrBedRoom",
    "nbrRoomsNobath", "mainfloorSF", "nbrFullBaths", "TotalFinishedSF",
    "Age", "schools_nn3")
...
# excluding boulder.sf$designCodeDscr and boulder.sf$qualityCodeDscr for now
# because they were returning errors
## SOMETHING STRANGE IS HAPPENING: boulder.training much larger than boulder.sf
inTrain <- createDataPartition(
              y = c(boulder.sf$price, boulder.sf$nbrBedRoom,
                    boulder.sf$nbrRoomsNobath, boulder.sf$mainfloorSF,
                    boulder.sf$nbrFullBaths, boulder.sf$TotalFinishedSF,
                    boulder.sf$Age, boulder.sf$schools_nn3,
                    boulder.sf$med_HH_Income, boulder.sf$pct.over75K),
              p = 0.75, list = FALSE)
...
mean(mean(boulder.test$price.ape, na.rm = T))
